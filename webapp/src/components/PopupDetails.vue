    ...
    <q-dialog v-model="model" ref="testRef">
        <q-card style="width: 700px; max-width: 80vw;">
            <q-toolbar>
                ...
                <q-btn flat v-close-popup round dense class="absolute-right" style="padding-right: 0.75vw;" icon="close" />
            </q-toolbar>
            <q-card-section>
                <div
                ...
                >
                    <q-list>
                        <q-item clickable :v-ripple="model" style="padding: 0%">
                            <list-element :detail="detail" :is-criteria="isCriteria" :goTo="goToAction"/>
                        </q-item>
                        <q-separator v-if="index !== details.length - 1" spaced inset />
                    </q-list>
                </div>
            </q-card-section>
        </q-card>
...
const otherModel = computed({
    get: () => props.otherModelValue,
    set: (value) => emits("update:otherModelValue", value),
});
